import { expertiseMeta } from '@tk/data'
...
type CardMessage = { title: string; subtitle: string; description: string }

export async function ExpertiseSection() {
  const t = await getTranslations('Expertise')
  const cards = t.raw('cards') as CardMessage[]
              ...
              >
                ...
                <div className="flex items-start gap-4">
                  <div className="text-(--card-accent)">
                    {iconMap[meta.icon] ?? <Monitor className="w-12 h-12" />}
                  </div>
